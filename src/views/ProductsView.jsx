...
class ProductsView extends Component {
  ...
  getProducts = (object) => {
    ...
    const prices = document
      .getElementsByName("product-price")[0]
      .value.split(",");
    const queryObj = {
    ...
    if (queryObj.PriceFrom > queryObj.PriceTo)
      [queryObj.PriceFrom, queryObj.PriceTo] = [
        queryObj.PriceTo,
        queryObj.PriceFrom,
      ];
    window
      .ProductAPIsService_Search(queryObj)
      .then((result) =>
        this.setState({
  ...
  handleDeleteSelected = async () => {
    ...
    for (let index = 0; index < this.state.selected.length; index++) {
      ...
      try {
        const result = await window.ProductAPIsService_Delete(
          id,
    ...
    if (isSuccess)
      ...
      notify(
        "Error",
        "Delete selected product(s) has one or more error",
        "error"
      );
  ...
  render() {
    ...
    const productCode = this.state.selected[0]
      ? getProductsByID(this.state.products, this.state.selected[0]).code
      : "PRO00000";
    return (
      <div className={classes.root}>
        <div className={classes.row}>
          ...
          <Link
            to={`/admin/products/update-${productCode}`}
            style={{ textDecoration: "none" }}
          >
            <Button
              variant="text"
              className={classes.toolbarBtn}
              disabled={1 !== this.state.selected.length}
            >
              Update
            </Button>
          </Link>
